...
const useIndustryMetrics = (data: DataItem[], industry: string) => {
  ...
  const highestPositiveChangeItem = useMemo(() => {
    const positiveChanges = industryData.filter((item) => item.LastTradedPrice_change > 0);
  ...
  const highestNegativeChangeItem = useMemo(() => {
    const negativeChanges = industryData.filter((item) => item.LastTradedPrice_change < 0);
